"""
Functions for performing non-metric multidimensional scaling (nMDS). These functions are originally from the PyCogent
package, and are reproduced here with modifications for the following reason(s):

* PyCogent does not play well with Windows, but seq-experiment should, so isolating this scikit-bio functionality
 allows us to make use of it without abandoning support for an entire operating system

The intended input for the NMDS class is a distance matrix/dissimilarity matrix as a numpy array like object.

For original license information see `licenses/PyCogent.txt`.
